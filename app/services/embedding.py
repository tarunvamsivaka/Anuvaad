...
async def generate_embeddings_hf(texts: list[str]) -> list[list[float]]:
    ...
    if not hf_token:
        logger.warning(
            "HF_TOKEN not found, attempting to use unauthenticated HF inference API (may be heavily rate-limited)."
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                HF_API_URL, headers=headers, json={"inputs": texts, "options": {"wait_for_model": True}}, timeout=30.0
            )
