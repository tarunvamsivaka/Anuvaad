...
// We duplicate the logic here rather than import from page.tsx because
// page.tsx is a "use client" module with heavy UI dependencies.
// TODO: Move detectLanguage to @/lib/detect-language.ts so it can be
//       imported directly — that refactor is tracked in the backlog.
...
function detectLanguage(code: string): string | null {
  ...
  if (
    /def\s+[a-zA-Z_]\w*\s*\(|import\s+[a-zA-Z_]\w*(?:\s*,\s*[a-zA-Z_]\w*)*\n|#\s+.*|elif\s+|if\s+__name__\s*==\s*['"]__main__['"]/.test(
      code
    )
  ) {
    return "python";
  ...
  if (
    /import\s+.*\s+from\s+['"]react['"]|const\s+\[\w+,\s*set\w+\]\s*=\s*useState|export\s+default\s+function|interface\s+[A-Z]\w*\s*\{|type\s+[A-Z]\w*\s*=/.test(
      code
    )
  ) {
    return "typescript";
  ...
  if (
    /console\.log\(|const\s+\w+\s*=\s*\(.*\)\s*=>|let\s+\w+\s*=|var\s+\w+\s*=/.test(
      code
    )
  ) {
    return "javascript";
  ...
  if (
    /fn\s+main\(\)|pub\s+struct\s+[A-Z]|impl\s+[A-Z]|let\s+mut\s+\w+|use\s+std::/.test(
      code
    )
  ) {
    return "rust";
  ...
  if (
    /#include\s*<[a-z]+>|std::cout|int\s+main\(\s*\)|using\s+namespace\s+std;/.test(
      code
    )
  ) {
    return "cpp";
  ...
  if (
    /public\s+class\s+[A-Z]|public\s+static\s+void\s+main|System\.out\.println/.test(
      code
    )
  ) {
    return "java";
